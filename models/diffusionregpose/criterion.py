...
class SetCriterion(nn.Module):
    ...
    def loss_keypoints(self, outputs, targets, indices, num_boxes):
        ...
        if len(src_keypoints) == 0:
            ...
            losses = {
                ...
                # 'loss_graph': torch.as_tensor(0., device=device) + src_keypoints.sum() * 0,
            }
            return losses
        ...
        losses['loss_oks'] = oks_loss.sum() * 1 / num_boxes
        # losses['loss_graph'] = graph_loss.sum() / num_boxes
        return losses
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        # loss_siou = 1 - self.shape_iou(box_ops.box_cxcywh_to_xyxy(src_boxes),
        #     box_ops.box_cxcywh_to_xyxy(target_boxes))
        losses['loss_giou'] = loss_giou.sum() / num_boxes
        # losses['loss_giou'] = loss_siou.sum() / num_boxes
    ...
    def loss_init_boxes(self, outputs, targets, indices, num_boxes):
        """Compute the losses related to the bounding boxes, the L1 regression loss and the GIoU loss
           targets dicts must contain the key "boxes" containing a tensor of dim [nb_target_boxes, 4]
           The target boxes are expected in format (center_x, center_y, w, h), normalized by the image size.
        """
        ...
        assert 'pred_boxes' in outputs
        idx = self._get_src_permutation_idx(indices)
        src_boxes = outputs['pred_init_boxes'][idx]
        target_boxes = torch.cat([t['boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0)
        loss_bbox = F.l1_loss(src_boxes, target_boxes, reduction='none')
        losses = {}
        losses['loss_init_bbox'] = loss_bbox.sum() / num_boxes
        loss_giou = 1 - torch.diag(box_ops.generalized_box_iou(
            box_ops.box_cxcywh_to_xyxy(src_boxes),
            box_ops.box_cxcywh_to_xyxy(target_boxes)))
        # loss_giou = 1 - self.shape_iou(box_ops.box_cxcywh_to_xyxy(src_boxes),
        #     box_ops.box_cxcywh_to_xyxy(target_boxes))
        losses['loss_init_giou'] = loss_giou.sum() / num_boxes
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            "init_boxes": self.loss_init_boxes,
    ...
    def forward(self, outputs, targets, return_indices=False):
        ...
        if return_indices:
            ...
            indices_list = []
        losses = {}
        # self.losses = ['labels', 'boxes', 'keypoints', 'matching', 'dn_label', 'dn_bbox', 'init_boxes']
        self.losses = ['labels', 'boxes', 'keypoints', 'matching', 'dn_label', 'dn_bbox']
        ...
        # interm_outputs loss
        if 'interm_outputs' in outputs: # 900
            interm_outputs = outputs['interm_outputs']
            indices = self.matcher(interm_outputs, targets)
            if return_indices:
                indices_list.append(indices)
            for loss in self.losses:
                if loss in ['dn_bbox', 'dn_label', 'keypoints', 'init_boxes']:
                    continue
                kwargs = {}
                if loss == 'labels':
                    kwargs = {'log': False}
